    ...
    for (int n1 = 0; n1 < N1; n1++) {
        for (int n2 = 0; n2 < N2; n2++) {
            for (int n3 = 0; n3 < N3; n3++) {
                sum = 0.0;
                count = 0;
                #pragma omp parallel for reduction(+:sum, count) default(shared) firstprivate(n1, n2, n3) collapse(3)
...
void stat(double *stats, const double u[N1][N2][N3]) {
    ...
    #pragma omp parallel for default(none) shared(u, mean) reduction(+:uvar) collapse(3)
    for (int n1 = 0; n1 < N1; n1++) {
        for (int n2 = 0; n2 < N2; n2++) {
            for (int n3 = 0; n3 < N3; n3++) {
